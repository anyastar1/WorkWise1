...
Проанализируй данные документа на соответствие ГОСТ 7.32-2001. 
...
{{
  ...
  "errors": <строка с ошибками по всему документу, например не хватает титульного листа или оглавления>,
...
def validate_gost_json(data: Dict) -> tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if "errors" not in data:
            return False, "Отсутствует поле 'errors'"
        ...
        # Проверяем тип verdict
        if not isinstance(data["errors"], str):
            return False, "Поле 'errors' должно быть строкой"
        ...
        for i, page in enumerate(data["pages"]):
            ...
            for field in required_fields:
                if field not in page:
                    return (
                        False,
                        f"Отсутствует поле '{field}' в элементе {i} массива 'pages'",
                    )
    ...
    def _worker(self):
        ...
        while self.running:
            try:
                ...
                if task["type"] == "page":
                    self._process_page(
                        task["page_id"], task["image_path"], task["page_number"]
                    )
    ...
    def _process_page(self, page_id: int, image_path: str, page_number: int):
        ...
        try:
            ...
            if not response_text:
                ...
                print(
                    f"[DEBUG] Ключи в ответе: {list(response.keys()) if isinstance(response, dict) else 'N/A'}"
                )
            ...
            for parse_attempt in range(1, max_parse_attempts + 1):
                try:
                    print(
                        f"[INFO] Попытка парсинга XML {parse_attempt}/{max_parse_attempts} для страницы {page_number}..."
                    )
                    ...
                    if not is_valid:
                        print(
                            f"[WARNING] Невалидный XML (попытка {parse_attempt}): {error_msg}"
                        )
                        if parse_attempt < max_parse_attempts:
                            ...
                            raise XMLParseError(
                                f"Невалидный XML после {max_parse_attempts} попыток: {error_msg}"
                            )
                    ...
                    last_error = e
                    print(
                        f"[WARNING] Ошибка парсинга XML (попытка {parse_attempt}/{max_parse_attempts}): {str(e)}"
                    )
...
Исправленный XML:"""
                            ...
                            correction_response = self.ollama_client.generate(
                                ...
                                + response_text[
                                    :2000
                                ],  # Первые 2000 символов для контекста
                            ...
                            correction_text = ""
                            if "message" in correction_response and isinstance(
                                correction_response["message"], dict
                            ):
                                correction_text = correction_response["message"].get(
                                    "content", ""
                                )
                            ...
                            if correction_text:
                                print(
                                    f"[INFO] Получен исправленный ответ, длина: {len(correction_text)}"
                                )
                                response_text = correction_text
                                continue  # Пробуем снова с исправленным ответом
                            else:
                                print(
                                    f"[WARNING] Не удалось получить исправленный ответ"
                                )

                        except Exception as correction_error:
                            print(
                                f"[WARNING] Ошибка при запросе исправления: {correction_error}"
                            )
            ...
            if blocks_data is None:
                raise XMLParseError(
                    f"Не удалось распарсить XML после {max_parse_attempts} попыток: {last_error}"
                )
            ...
            error_msg = str(e)
            print(
                f"[ERROR] Ошибка обработки страницы {page_id} (страница {page_number}): {error_msg}"
            )
            import traceback
    ...
    def _check_all_pages_processed(self, document_id: int, db):
        ...
        if all_completed and not document.all_pages_processed:
            ...
            threading.Thread(
                target=self._check_gost_compliance, args=(document_id,), daemon=True
            ).start()
    ...
    def _check_gost_compliance(self, document_id: int):
        ...
        try:
            ...
            if not document or document.gost_check_completed:
                logger.warning(
                    f"[GOST] Документ {document_id} не найден или проверка уже завершена"
                )
                return
            ...
            logger.info(
                f"[GOST] XML агрегирован, длина: {len(aggregated_xml)} символов"
            )
            ...
            for attempt in range(1, max_attempts + 1):
                logger.info(
                    f"[GOST] Попытка {attempt}/{max_attempts} получения ответа от Ollama"
                )
                ...
                logger.info(
                    f"[GOST] Получен ответ от Ollama, длина: {len(raw_response)} символов"
                )
                ...
                if not json_str:
                    ...
                    if attempt < max_attempts:
                        ...
                        logger.error(
                            f"[GOST] Не удалось извлечь JSON после {max_attempts} попыток"
                        )
                ...
                try:
                    ...
                    logger.warning(
                        f"[GOST] Попытка {attempt}: Ошибка парсинга JSON: {e}"
                    )
                    if attempt < max_attempts:
                        logger.info(f"[GOST] Повторная попытка...")
                        continue
                        ...
                        logger.error(
                            f"[GOST] Не удалось распарсить JSON после {max_attempts} попыток"
                        )
                        report_text = raw_response
                        break
                ...
                if not is_valid:
                    logger.warning(
                        f"[GOST] Попытка {attempt}: JSON не прошел валидацию: {error_msg}"
                    )
                    if attempt < max_attempts:
                        logger.info(f"[GOST] Повторная попытка...")
                        continue
                        ...
                        logger.error(
                            f"[GOST] JSON не прошел валидацию после {max_attempts} попыток: {error_msg}"
                        )
                        # Сохраняем JSON как есть, но с предупреждением
                        report_text = json.dumps(
                            gost_data, ensure_ascii=False, indent=2
                        )
                        break
            ...
            if gost_data:
                ...
                if report_text:
                    if (
                        "соответствует" in report_text.lower()
                        and "не" not in report_text.lower()[:50]
                    ):
                        verdict = "да"
                    elif (
                        "не соответствует" in report_text.lower()
                        or "наруш" in report_text.lower()[:100]
                    ):
                        verdict = "нет"
            ...
            if gost_data and "pages" in gost_data:
                logger.info(
                    f"[GOST] Начало создания визуализации ошибок для документа {document_id}"
                )
                try:
                    ...
                    pages_data_dict = {
                        pg_data.get("page"): pg_data for pg_data in pages_data
                    }
                    ...
                    for page in pages:
                        ...
                        if (
                            page_gost_data
                            and page_gost_data.get("gosterror", "").strip()
                        ):
                            ...
                            if (
                                ...
                                if os.path.exists(image_path):
                                    ...
                                    vis_path = os.path.join(
                                        base_dir, f"{name}_gost{ext}"
                                    )

                                    # Создаем визуализацию
                                    try:
                                        visualize_gost_errors(
                                            image_path, [page_gost_data], vis_path
                                        )
                                        logger.info(
                                            f"[GOST] Визуализация создана для страницы {page.page_number}: {vis_path}"
                                        )
                                    except Exception as vis_error:
                                        logger.error(
                                            f"[GOST] Ошибка создания визуализации для страницы {page.page_number}: {vis_error}"
                                        )
                                        import traceback

                                        logger.error(
                                            f"[GOST] Трассировка: {traceback.format_exc()}"
                                        )
                                else:
                                    logger.warning(
                                        f"[GOST] Изображение страницы {page.page_number} не найдено: {image_path}"
                                    )

                    logger.info(
                        f"[GOST] Визуализация ошибок завершена для документа {document_id}"
                    )
                except Exception as vis_exception:
                    logger.error(
                        f"[GOST] Ошибка при создании визуализации для документа {document_id}: {vis_exception}"
                    )
                    import traceback
            ...
            logger.error(
                f"[GOST] Ошибка проверки ГОСТ для документа {document_id}: {e}"
            )
            import traceback
