...
if not GEMINI_API_KEY:
    GEMINI_API_KEY = "AIzaSyC-7i_dvNeqfPGu1_47_9rwsq9tOM4rkzQ"

GEMINI_MODEL = "gemini-2.0-flash"
GEMINI_API_BASE_V1BETA = "https://generativelanguage.googleapis.com/v1beta"
GEMINI_API_BASE_V1 = "https://generativelanguage.googleapis.com/v1"
...
IS_API_CONFIGURED = is_api_key_valid(GEMINI_API_KEY)

if IS_API_CONFIGURED:
    print(f"✅ Google Gemini API настроен: {GEMINI_API_KEY[:10]}... {GEMINI_API_KEY[-4:]}")
else:
    print("⚠️  ВНИМАНИЕ: API ключ Google Gemini не настроен!")
    print("   Установите переменную окружения GEMINI_API_KEY или измените значение в app.py")
...
def call_gemini_api(prompt, system_instruction=None, max_output_tokens=4000, temperature=0.3):
    """
    Вызывает Google Gemini API с заданным промптом. 
    ...
    if not IS_API_CONFIGURED:
        raise ValueError("API ключ Google Gemini не настроен.  Установите GEMINI_API_KEY.")
    ...
    test_url = f"{GEMINI_API_BASE_V1BETA}/models?key={GEMINI_API_KEY}"
    try:
        test_response = requests. get(test_url, timeout=10)
        if test_response.status_code == 200:
            available_models = test_response.json(). get('models', [])
            model_names = [m. get('name', '').split('/')[-1] for m in available_models]
            print(f"✅ Доступные модели: {', '.join(model_names[:5])}...")
        else:
            print(f"⚠️  Не удалось получить список моделей (код: {test_response. status_code})")
    except Exception as e:
        print(f"⚠️  Ошибка при проверке доступности API: {e}")
    ...
    api_bases = [GEMINI_API_BASE_V1BETA, GEMINI_API_BASE_V1]
    ...
    for api_base in api_bases:
        ...
        for model in models_to_try:
            try:
                current_url = f"{api_base}/models/{model}:generateContent?key={GEMINI_API_KEY}"
                print(f"📤 Попытка подключения (API: {api_base. split('/')[-1]}, модель: {model})...")
                ...
                if response. status_code == 200:
                    print(f"✅ Успешное подключение к модели: {model}")
                ...
                if response.status_code == 404:
                    ...
                    try:
                        ...
                        pass
                    print(f"⚠️  Модель {model} недоступна (404)")
                    if error_detail:
                        print(f"   Детали ошибки: {error_detail}")
                    last_error = f"Модель {model} не найдена (404)"
                    continue
                ...
                if response.status_code == 403:
                    ...
                    try:
                        ...
                        pass
                    error_msg = f"Доступ запрещен (403).  Проверьте API ключ и разрешения."
                    if error_detail:
                        error_msg += f" Детали: {error_detail}"
                ...
                print(f"⚠️  Ошибка при запросе к модели {model}: {e}")
                last_error = str(e)
                continue
    ...
    if not last_response or last_response.status_code != 200:
        error_details = []
        if last_response:
            error_details.append(f"Последний статус: {last_response. status_code}")
            try:
                error_data = last_response.json()
                api_error = error_data. get('error', {})
                if api_error:
                    error_details.append(f"Сообщение API: {api_error.get('message', 'Нет деталей')}")
                    error_details.append(f"Статус: {api_error.get('status', 'Неизвестно')}")
            except:
                error_details.append(f"Ответ сервера: {last_response.text[:200]}")
        
        error_msg = f"Не удалось подключиться ни к одной модели Gemini API. Последняя ошибка: {last_error}"
        raise ValueError(error_msg)
    ...
    if response.status_code == 400:
        ...
        error_msg = error_data.get('error', {}).get('message', 'Неверный запрос к Gemini API')
        raise ValueError(f"Ошибка запроса: {error_msg}")
    elif response.status_code == 401:
        raise ValueError("Неверный API ключ Google Gemini.")
    elif response. status_code == 404:
        raise ValueError(f"Модель не найдена.")
    elif response. status_code == 429:
        raise ValueError("Превышен лимит запросов к Gemini API.")
    elif response.status_code >= 500:
        raise ValueError(f"Ошибка сервера Google Gemini (код {response.status_code}).")
    ...
    try:
        ...
        if 'candidates' in data and len(data['candidates']) > 0:
            ...
            if 'content' in candidate and 'parts' in candidate['content']:
                ...
                if len(parts) > 0 and 'text' in parts[0]:
                    ...
                    if content:
                        print(f"✅ Получен ответ от Gemini API ({len(content)} символов)")
                        return content
                    else:
            ...
            if 'finishReason' in candidate:
                ...
                if finish_reason == 'SAFETY':
                    raise ValueError("Ответ заблокирован системой безопасности Gemini.")
                elif finish_reason == 'RECITATION':
                    raise ValueError("Ответ заблокирован из-за возможного копирования контента.")
        ...
        raise ValueError(f"Ошибка подключения к Google Gemini API: {str(e)}")
    except json.JSONDecodeError:
        raise ValueError("Некорректный JSON ответ от API")
    except ValueError as e:
        raise
    except Exception as e:
        raise ValueError(f"Ошибка при вызове API: {str(e)}")


def call_gemini_api_with_images(prompt: str, system_instruction: str, 
                                 images_base64: List[str], 
                                 max_output_tokens: int = 8000,
                                 temperature: float = 0.1) -> str:
    """
    Вызывает Gemini API с изображениями. 
    ...
    """
    if not IS_API_CONFIGURED:
        raise ValueError("API ключ Google Gemini не настроен.")
    
    # Формируем контент с изображениями
    parts = []
    
    # Добавляем изображения (максимум 16 для API)
    max_images = min(len(images_base64), 16)
    for i, img_base64 in enumerate(images_base64[:max_images]):
        parts.append({
            "inline_data": {
                "mime_type": "image/png",
                "data": img_base64
        ...
        print(f"📷 Добавлено изображение {i + 1}/{max_images}")
    ...
    # Используем модели с поддержкой vision
    ...
    for api_base in api_bases:
        for model in models_to_try:
            try:
                current_url = f"{api_base}/models/{model}:generateContent?key={GEMINI_API_KEY}"
                print(f"📤 Попытка с изображениями: {model}...")
                
                response = requests.post(current_url, headers=headers, json=payload, timeout=180)
    ...
    if not last_response or last_response.status_code != 200:
        raise ValueError(f"Не удалось отправить изображения в Gemini API: {last_error}")
...
Верни JSON:
{
    ...
    try:
        response_text = call_gemini_api_with_images(prompt, system_instruction, images_base64)
...
{{
    ...
    try:
        response_text = call_gemini_api(prompt, system_instruction, max_output_tokens=8000, temperature=0.1)
        cleaned_response = clean_json_response(response_text)
...
def check_file():
    ...
    if request.method == 'POST':
        ...
        # Проверка API
        if not IS_API_CONFIGURED:
            flash('Ошибка: API ключ Google Gemini не настроен', 'error')
            db.close()
            return redirect(request.url)
        ...
        try:
            ...
            print(f"🔍 Начинаем анализ файла: {filename}, ГОСТ ID: {gost_id}")
