...
class User(Base):
    ...
    uploads = relationship(
        "UserUpload", back_populates="user", order_by="UserUpload.upload_date.desc()"
    )
...
def initialize_database():
    ...
    if os.path.exists(db_file_path):
        print(
            f"Обнаружен старый файл БД '{db_file_path}'. Удаляю для чистого старта MVP..."
        )
        try:
    ...
    if session.query(User).count() == 0:
        ...
        gost_id_1 = (
            session.query(GOST.id)
            .filter_by(name="ГОСТ Р 7.0.5-2008 (Библиографические ссылки)")
            .one()[0]
        )
