    ...
    def check(self, document_structure: dict) -> RuleResult:
        ...
        for page in document_structure.get("pages", []):
            ...
            for block in page.get("blocks", []):
                ...
                if x1 > page_width:
                    issues.append(
                        f"выходит за правую границу ({x1:.1f} > {page_width:.1f})"
                    )
                if y1 > page_height:
                    issues.append(
                        f"выходит за нижнюю границу ({y1:.1f} > {page_height:.1f})"
                    )
    ...
    def check(self, document_structure: dict) -> RuleResult:
        ...
        for page in document_structure.get("pages", []):
            ...
            for block in page.get("blocks", []):
                ...
                avg_font_size = sum(
                    s.get("style", {}).get("font_size", 14) for s in spans
                ) / len(spans)
                normalized_spacing = (
                    avg_spacing / avg_font_size if avg_font_size > 0 else 0
                )

                if (
                    normalized_spacing < self.min_spacing
                    or normalized_spacing > self.max_spacing
                ):
                    bbox = block.get("bbox", {})
                    errors.append(
                        RuleError(
...
class HeadingRule(BaseRule):
    ...
    def check(self, document_structure: dict) -> RuleResult:
        ...
        for page in pages_to_check:
            ...
            headings = [
                b for b in page.get("blocks", []) if b.get("block_type") == "heading"
            ]
...
class PageNumberRule(BaseRule):
    ...
    def _find_page_number(self, blocks: List[dict]) -> Optional[dict]:
        ...
        for block in blocks:
            ...
            # Проверяем позицию
            if not (
                self.position_y_min - self.tolerance
                <= y0
                <= self.position_y_max + self.tolerance
            ):
                continue
    ...
    def check(self, document_structure: dict) -> RuleResult:
        ...
        return RuleResult(
            ...
            passed=len(errors) == 0,
            errors=errors,
            stats={"start_page": self.start_page, "start_number": self.start_number},
        )
